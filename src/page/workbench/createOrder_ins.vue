<template>
    <div class="add-client-box fillcontain DN" v-loading="loading">
        <div class="pagetab">
            <ul>
                <li>
                    <button @click="$router.replace('/manage/createOrder_fix')">维修工单</button>
                </li>
                <li>
                    <button @click="$router.replace('/manage/createOrder_ins')" class="active">保险工单</button>
                </li>
            </ul>
        </div>
        <header class="header">
        ...
        <div class="baseform" v-show="baseActive">
            ...
            <div class="el-row">
                <div class="el-col el-col-12">
                    ...
                    <el-button type="text" @click="dialogFormVisible=true">新增保险公司</el-button>
                </div>
                <div class="el-col el-col-12">
                    <div class="base_hd"><em>*</em>是否自费：</div>
                    <el-checkbox-group  v-model="self_fee">
                        <el-checkbox :label="self_fee==false?'否':'是'" name="take"></el-checkbox>
                    </el-checkbox-group>
                </div>
            </div>
            <div class="el-row">
        ...
        <el-dialog title="新增保险公司" :visible.sync="dialogFormVisible" width="20%" top="35vh">
...
<script>
    import {getOrder,editOrder,getProjects,getTeam,getEmployee,quick_order_car,quick_order,getParts,getInsurance,addInsurance} from '@/api/getData'
    export default {
        data() {
            return {
                contacts:'',
        ...
        methods: {
            ...
            async remoteMethod(query) {
                if (query !== '') {
                    this.loading = true;
                    const res = await getInsurance({where:query.toLowerCase(),timestamp:new Date().getTime()})
                    if(res.state_code == 'ok'&& res.results != []){
                        this.loading = false
                        this.options4 = res.results
                    }else{
                        this.loading =false
                        this.$message.error('res.err_message');
                    }
            ...
            deletePro(item){
                ...
                this.$confirm('确认删除？')
                    .then(_ => {
                        ...
                        this.handleProCount();
                        console.log(this.maintenance_project_ids)
                    })
                    .catch(_ => {});
            },
            //删除选中配件
            deletePart(item){
                ...
                this.$confirm('确认删除？')
                    .then(_ => {
                        ...
                        this.handlePartsCount();
                        console.log(this.required_accessories_ids)
                    })
                    .catch(_ => {});
            },
